...
- npm / npx インストールでは、現在のプラットフォーム用バイナリを起動するために Node.js 18 以上が必要です。
- ローカル開発とバイナリのビルドには Bun が必要です。npm のプラットフォームパッケージと GitHub Release バイナリには Bun ランタイムが含まれています。

## npm インストール

npm パッケージ名は `@quietlychan/repo-sweep` です。インストール後のコマンド名は `repo-sweep` のままです。

npm は `@quietlychan/repo-sweep-darwin-arm64` など、現在のプラットフォームに合うバイナリパッケージを自動で取得します。Node.js は薄い launcher にだけ使われ、CLI 本体は Bun ネイティブ API の単一実行ファイルとして動作します。
...
Node.js を使わない場合は、GitHub Releases から現在のプラットフォーム向けの単一実行ファイルをダウンロードしてください。
...
以下の例は、npm でグローバルインストール済み、または GitHub Release のバイナリを使う前提です。一時的に `npx` で実行する場合は、`repo-sweep` を `npx @quietlychan/repo-sweep` に置き換えてください。
